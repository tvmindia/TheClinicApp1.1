    ...
    public class ImageHandler : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            try
            {
                ...
                if ((context.Request.QueryString["ClinicLogoID"] != null) && (context.Request.QueryString["ClinicLogoID"] != ""))
                {
                    TheClinicApp1._1.ClinicDAL.Clinic ClinicObj = new TheClinicApp1._1.ClinicDAL.Clinic();
                    //DAL.Boutiques BouObj = new DAL.Boutiques();
                    ClinicObj.ClinicID = Guid.Parse(context.Request.QueryString["ClinicLogoID"]);
                    byte[] productimg = ClinicObj.GetClinicLogo();
                    if (productimg != null)
                    {
                        MemoryStream memoryStream = new MemoryStream(productimg, false);
